#!/usr/bin/env python3
"""
Fortnite SDK Converter - Fixed for your specific Dumper-7 format
"""
...
class FortniteSDKConverter:
    ...
    def convert_sdk_directory(self, sdk_path: str) -> None:
        """Convert all .h files in the SDK directory"""
    ...
    def parse_dumper7_format(self, content: str, filename: str) -> List[Dict[str, Any]]:
        """Parse Dumper-7 specific format"""
        classes = []
        
        # Look for the class definition pattern that Dumper-7 uses
        # Pattern: class/struct ClassName final : public ParentClass
        ...
        for match in matches:
            ...
            # Calculate class size from members or look for size comments
    ...
    def parse_members_dumper7(self, class_body: str) -> List[Dict[str, str]]:
        ...
        for line in lines:
            ...
            # Skip access modifiers and other non-member lines
            if line in ['public:', 'private:', 'protected:'] or line.startswith('//'):
                continue
            
            # Look for member variable pattern
            # Common Dumper-7 patterns:
            # Type MemberName; // 0x0000(0x0008)
            # Type MemberName; // Offset: 0x0000
            member_patterns = [
                # Standard pattern: Type Name; // 0x0000(0x0008)
                r'^([A-Za-z0-9_:<>,\s\*\[\]&]+)\s+([A-Za-z0-9_]+);\s*//\s*0x([0-9A-Fa-f]+)\(0x([0-9A-Fa-f]+)\)',
                # Alternative: Type Name; // 0x0000 (0x0008)
                r'^([A-Za-z0-9_:<>,\s\*\[\]&]+)\s+([A-Za-z0-9_]+);\s*//\s*0x([0-9A-Fa-f]+)\s*\(0x([0-9A-Fa-f]+)\)',
                # Simple offset only: Type Name; // 0x0000
    ...
    def guess_type_size(self, type_name: str) -> str:
        ...
        # Clean type name for lookup
        clean_type = type_name.strip().lower()
        
        # Check for pointers
        if '*' in type_name:
            return '08'  # Pointer size on 64-bit
        
        # Check for arrays
        if '[' in type_name:
            return '04'  # Default array element size
        
        # Look up common types
    ...
    def calculate_class_size(self, content: str, class_name: str, members: List[Dict]) -> int:
        ...
        # Look for size comments in the content
        size_patterns = [

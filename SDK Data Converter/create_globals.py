#!/usr/bin/env python3
"""
Create globals.json from Fortnite offsets.h
Extracts global addresses and common offsets not typically in SDK dumps
"""
...
def create_globals_json():
    """Create globals.json with offsets from your offsets.h"""
    
    # Global base addresses (these are never in SDK dumps)
    global_bases = {
        "UWorld": "0x173BDD18",
        "GNames": "0x167B6600"
    }
    
    # Engine/Game offsets that are usually not in SDK member definitions
    # These are runtime offsets for accessing game objects
    engine_offsets = {
        "UWorld": {
    ...
    # Additional common offsets for reverse engineering
    common_offsets = {
        "UObject": {
            "Class": "0x10",
            "Name": "0x18",
...
def update_existing_globals():
    """Update existing globals.json if it exists, otherwise create new one"""
    ...
    if os.path.exists(globals_file):
        ...
        # Merge bases (overwrite with new values)
        if 'bases' not in existing_data:
            existing_data['bases'] = {}
        existing_data['bases'].update(new_data['bases'])
        
        # Merge offsets (add new classes, update existing)
        ...
        for class_name, class_offsets in new_data['offsets'].items():
            if class_name not in existing_data['offsets']:
                ...
                # Update existing class with new offsets
